...
def ingest_reading(packet: dict, request: Request):
    try:
    ...
    store.open_incident(decision)
    return {"id": reading_id, "packet": packet, "decision": decision}
...
def overview(request: Request):
    ...
    return {
        ...
        "units": [
            {"id": "ALPHA", "status": "AVAILABLE", "position": [12, 82]},
            {"id": "BRAVO", "status": "AVAILABLE", "position": [88, 18]},
        ],
    }


@router.post("/demo/{scenario}")
def run_demo(scenario: str, request: Request):
...
def dispatch(incident_id: int, request: Request, unit: str = "ALPHA"):
